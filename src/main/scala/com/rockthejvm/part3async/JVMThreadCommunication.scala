...
object ProdConsV1 {
  def start(): Unit = {
    ...
    val consumer = new Thread(() => {
      ...
      // busy waiting - blocks the processor for as long as the thread is alive...
...
object ProdConsV2 {
  def start(): Unit = {
    ...
    val consumer = new Thread(() => {
      ...
      container.synchronized { // block all other threads to "lock" this object
        // thread-safe code
        if (container.isEmpty)
          container.wait() // releases the lock of the object you just locked and suspends the thread
        println(s"[consumer] I have consumed a value: ${container.get}")
      }
...
object ProdConsV3 {
  def start(containerCapacity: Int): Unit = {
    val buffer: mutable.Queue[Int] = mutable.Queue(containerCapacity)
    ...
    val consumer = new Thread(() => {
      ...
      while(true) {
        buffer.synchronized {
          ...
          buffer.notify() // to wake up the producer if it's asleep
        }
        ...
        buffer.synchronized {
          /*
            we need to constantly check if the buffer is empty - scenario:
              ...
              consumer dequeues, calls notify, awakens the other consumer(instead of producer...)
          ...
          // notify a producer
          /*
            We need to use nofityAll, Otherwise:
